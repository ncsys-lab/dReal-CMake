...
void PreventUsingIbexCheck::registerMatchers(MatchFinder *Finder) {
  Finder->addMatcher(usingDecl(hasAnyUsingShadowDecl(
      hasTargetDecl(namedDecl(matchesName(".*ibex.*"))))).bind("usingDecl"),
    this);

  Finder->addMatcher(usingDirectiveDecl(hasAnyNamespaceRef(
      namespaceDecl(matchesName(".*ibex.*")))).bind("usingDirective"),
    this);
