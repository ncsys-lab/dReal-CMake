...
**Source-hygiene lint (`lint.py`) — there is no real clang-tidy.** This is a dependency-free regex check (the syntactic intent of a clang-tidy custom check, realized without the LLVM/libTooling build; no `.clang-tidy`, no `CMAKE_CXX_CLANG_TIDY`, nothing in CI). Run it with `python3 lint.py` (static, no build) or `./rounding_debug_gate.sh` (lint + Debug ctest gate). It forbids, in `src/dreal/`: **(routing)** raw `ibex::Function::backward` (route through `ibex_hc4_backward`), raw `.mid()`/`.diam()` (route through `safe_mid`/`safe_diam`) — all in `util/rounded_interval.h` — intervals built from hand-written scalar `+`/`-` arithmetic (use `make_sound_interval`), and raw json `.dump(` (route through `dump_json`, `util/json_guarded.h`); **(iteration shape)** a subscript by a side-effecting counter (`arr[i++]`/`arr[++i]`) — a parallel index that drifts out of sync with a range-`for` loop variable, the BUG-005 scrambled-model class (commit `5774191f2`, fixed `0296a8e19`); use one shared index. Scalar `double`→decimal routing (`format_double`) is **not** regex-detectable, so it is enforced at *compile time* by `format_double` requiring the `NearestRounding` token, not by the lint. Legitimate exceptions carry an inline `// lint: allow <reason>` marker; the parser/scanner sources and the wrapper-definition files (`rounded_interval.h`, `rounded_format.h`, `json_guarded.h`) are skipped.
