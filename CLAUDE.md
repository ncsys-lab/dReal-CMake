...
- `contractor_odes_codac.cc`: `CtcLohner` with `TimePropag::FWD_BWD` for integration; `LohnerAlgorithm` for trace/visualization. Per-flow `CodacOdeCache` (`AnalyticFunction` + `CtcLohner` reuse) keyed by `OdeFlow*` is shared across modes and parallel ICP workers. `contractions=2`, `eps=0.1`; BWD-direction contractor skips CtcLohner (Step 4) because the constructor's `m_vars_0 ↔ m_vars_t` swap makes it ask the wrong question — see `CODAC_MIGRATION.md` for the correctness analysis.
...
- Known performance gap on `bouncing_ball_with_drag_10_0.smt2`: ~2.8 s with Codac CtcLohner vs ~0.5 s (CAPD order-20, x86 Rosetta). Floor on bouncing-ball-class benchmarks; not fixable without Codac internals or a CAPD restoration. See `CODAC_MIGRATION.md` for the CAPD v6 ARM64 fix if this needs to change.
