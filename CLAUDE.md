...
- `contractor_odes_codac.cc`: `CtcLohner` with `TimePropag::FWD_BWD` for FWD integration; `LohnerAlgorithm(forward=false)` for BWD integration (one-shot backward image via `run_lohner_bwd_oneshot`); `LohnerAlgorithm` for trace/visualization. Per-flow `CodacOdeCache` (`AnalyticFunction` + `CtcLohner` reuse) keyed by `OdeFlow*` is shared across modes and parallel ICP workers. FWD uses `contractions=2`, `eps=0.1`; BWD uses LohnerAlgorithm's default `contractions=1`. The BWD direction was previously skipped (`CtcLohner` with the `m_vars_0 ↔ m_vars_t` constructor swap asked the wrong question); the LohnerAlgorithm restoration landed in the third optimization pass — see `CODAC_MIGRATION.md` for the soundness analysis and the unit-test verification.
- Trivial-flow short-circuit: flows where every RHS is the literal 0 bypass CtcLohner entirely and just intersect X_0 ∩ X_t (e.g. the d/dt[d]=0 planning benchmark with 1280 modes).
- Adaptive `n_steps` in `run_lohner_integration`: `n_steps = clamp(max(n_steps_hint, ceil(t_ub * 2)), n_steps_hint, 60)`. Keeps the historical 20-step floor for short horizons (bouncing ball unchanged) and only adds steps when t_ub > ~10 so long-horizon benchmarks like cardiac (t_ub up to 30) get h ≤ 0.5 instead of h = 1.5.
- `contractor_ibex_fwdbwd::Prune` uses an input-restricted snapshot (`thread_local std::vector<(int, ibex::Interval)>` saving only the constraint's free-var intervals) instead of a full-box copy + `std::set<int>`. Cost is O(|free_vars(f)|) instead of O(|box|); important because most non-ODE benchmarks spend the majority of theory-solver time in this Prune.
- Known performance gap on `bouncing_ball_with_drag_10_0.smt2`: ~4.2 s with Codac CtcLohner FWD + LohnerAlgorithm BWD vs ~0.5 s (CAPD order-20, x86 Rosetta). The ~1.5× slowdown from the BWD restoration is the cost of the added per-Prune backward integration; pre-BWD-restoration was ~2.8 s. Floor on bouncing-ball-class benchmarks; not fixable without Codac internals or a CAPD restoration. See `CODAC_MIGRATION.md` for the CAPD v6 ARM64 fix if this needs to change.
...
**ODE performance baseline** (`upgrade-ibex` branch, ARM64 macOS, after three optimization passes): `bouncing_ball_with_drag_10_0.smt2` (10 modes) takes ~4.2 s with Codac `CtcLohner` FWD (Taylor order 2, contractions=2, n_steps=20) + `LohnerAlgorithm(forward=false)` BWD (contractions=1). Pre-BWD-restoration was ~2.8 s; old CAPD (Taylor order 20, x86 Rosetta) took ~0.5 s. The ~8× gap is accepted because CAV26 research focus is pattern-matching/lemma reuse, not ODE integration speed. Long-horizon benchmarks (e.g. cardiac with t_ub up to 30) recovered to baseline thanks to the adaptive-`n_steps` heuristic; quad/crazyflie/prostate with complex 10+-var flows remain Codac-order-2-bound. See `CODAC_MIGRATION.md` for the CAPD v6 ARM64 fix if ODE speed becomes critical.
