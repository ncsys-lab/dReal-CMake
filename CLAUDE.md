...
## Soundness vs. completeness (terminology discipline — read before reporting either)

dReal is **sound but δ-complete**; these are not interchangeable, and conflating them has cost real experiments here (the hull-grid "F1" finding was filed as soundness when it was completeness, and got runs cancelled). The dichotomy, for this solver:

- **false `unsat`** (returns `unsat` on a (δ-)satisfiable φ) = a **SOUNDNESS** violation — the forbidden one. Over-approximating contractors never remove a real solution, so *looseness never causes this*; soundness breaks only where a box is narrowed *past* a true model (wrong FPU rounding, truncated ODE feed, strict-bound `nextafter`, scrambled model, underflow).
- **missed refutation / false `delta-sat`** (fails to refute a robustly — gap ≫ δ — infeasible φ) = a **COMPLETENESS** violation. A *looser* enclosure (lower `--ode-hull-grid`/`--ode-taylor-order`) weakens *this*, never soundness. Within the δ-band it is by design.

**Mandate:** whenever you report, label, comment, or commit a soundness/completeness issue — in chat, code, docs, or messages — **append the model-theory characterization in parentheses**, naming which T-relation the solver *asserts* vs. which is *true*. Never a bare "soundness bug"/"soundness gate". Templates: `SOUNDNESS (asserts φ T-unsatisfiable on a T-satisfiable φ — false unsat)`; `COMPLETENESS (asserts φ^δ T-satisfiable on a T-unsatisfiable φ — missed refutation)`. Canonical reference (T-relation defs, dReal's `unsat`/`δ-sat` guarantees, worked F1 example): `docs/soundness-vs-completeness.md`. Model-theory depth for the publication work: the `smt-model-theory` skill.
...
**CAPD ODE-contractor tuning flags** (runtime overrides for what were compile-time constants): `--ode-taylor-order` (fwd, default 12), `--ode-backward-order` (bwd, 12), `--ode-abs-tol`/`--ode-rel-tol` (1e-10), `--ode-hull-grid` (per-step tube sub-slices, 4), `--ode-c0-set` (`rect2`|`tripleton`|`horect2`), `--ode-backward` (enable X₀-narrowing bwd contractor, true), `--ode-max-step` (0 = adaptive). Threaded via `CapdSolverParams` (`ode_types.h`) → `run_capd_*`; defaults/rationale live on the `kDefaultOde*` constants in `solver/config.h`. The 2026-06 sweep retuned the defaults from order-20/hull-16 to order-12/hull-4 (~2× faster, +4 solved, zero verdict flips); campaign + per-knob/per-problem findings: `OPTIMIZATION_LOG.md` "2026-06 re-tuning campaign". **`--ode-hull-grid` is NOT a free speed knob — it is the time-resolution of interior-invariant refutation. Lowering it only widens enclosures (never a false-`unsat`) but trades away refutation *completeness* (a sharp interior violation below the resolution returns delta-sat instead of unsat — the accepted hull-4 tradeoff; `HULL_COMPLETENESS.md`). For refutation-critical / sharp-invariant problems, raise it (e.g. `--ode-hull-grid 16 --ode-taylor-order 20`). A deferred width-based sub-slicing fix would remove this coupling.**
...
5. **Contractors** (`src/dreal/contractor/`): ICP works by iterating contractors — algorithms that shrink variable domains. Key contractors:
   ...
   - `contractor_ode_lohner`: ODE contractor wrapping CAPD's order-20 `IOdeSolver` + `ITimeMap` (`contractor_odes_capd.{h,cc}`). The trivial-flow short-circuit (every RHS is literal 0) bypasses CAPD and just intersects X_0 ∩ X_t. `--visualize` produces step-by-step CAPD enclosures via `run_capd_trace`. CAPD divergence on a Prune call silently skips narrowing for that call. The previous Codac/CAPD gated hybrid was retired; see `docs/decisions.md`.
...
Codac and Eigen3 are no longer dependencies. See `DEPENDENCIES.md` for the current stack and `docs/decisions.md` "ODE backend" for the migration rationale.
...
- ODE contractor in `contractor_odes.cc`: `run_capd_fwd`/`run_capd_bwd` (CAPD `IOdeSolver` order-20, `kCapdTaylorOrder`) integrate `f(x)` / `-f(x)` and return the **time-ordered per-slice tube** (`kHullGrid=16` enclosures per adaptive step), NOT a coarse hull; `contractor_ode_lohner::Prune` runs the per-slice invariant/terminal-gate filter (no survivor → sound `set_empty()`). The per-slice form (vs the Codac→CAPD rewrite's single-endpoint intersection, which false-`unsat`s free-time integrals) is load-bearing — mechanism in `docs/ode-integration.md` §Mechanism, decision + proof case in `docs/decisions.md` "Per-slice ODE tube". Tests: `contractor_odes_semantic_test.cc`.
...
- `run_batch.sh` — parallel runner: reads TSV from stdin, runs each with `gtime -v -o`, `nice -n 1`, and `timeout 600`. Two env hooks: `DREAL_ARGS` injects per-invocation solver flags (so one binary can be swept over flag values); `TIMEOUT` overrides the 600 s cap. Throttle caps at 12 concurrent (count actual solvers with `pgrep -x dreal4`, NOT `pgrep -f gcc_build/dreal4` — the latter also matches the `gtime`/`nice`/`timeout` wrappers, ~3 per solve)
...
- `do_sweep.sh NAME1="flags1" NAME2="flags2" …` — sweeps **one** binary over many flag configs (e.g. `"base="` `"ord12=--ode-taylor-order 12"`) on the same jobs, for meta-parameter tuning. Unlike `do_ab` (sequential), it runs **all (config × benchmark) pairs in ONE shuffled 12-way pool** so a slow config's long-pole overlaps other configs' fast jobs — full CPU use, no idle tail, while still ≤`MAXJOBS` concurrent (per-process CPU-time stays accurate). Env: `JOBS` (default `probe_odes.tsv`), `DREAL_BINARY`, `MAXJOBS` (12), `TIMEOUT` (600; OFAT probe uses a tighter cap). Emits per-config `summary.csv` + a `compare_solvers.py` table. **Experimental design (OFAT → interactions → 123-job confirm) and the sweep metric/soundness-flip rules: `OPTIMIZATION_LOG.md` "2026-06 re-tuning campaign".**
...
**ODE feed faithfulness (`to_capd_string` precision)** — the field CAPD integrates must be a faithful image of the RHS; constants render at 17 sig figs (a 6-digit `std::to_string` truncation was a false-`unsat` soundness bug). Depth + the cav26-oracle A/B: `docs/decisions.md` "ODE feed faithfulness" and `docs/ode-integration.md` §Soundness.

**Denormal/underflow soundness (dreal/dreal4#321)** — `pow(0.5,1075)` underflows; made sound in two layers (ibex HC4-backward `underflow_saturate` + Drake `sound_constant_fold`). Full record incl. the accepted delta-completeness tradeoff (and the one UNSAT→delta-sat flip *not* to re-investigate): `docs/decisions.md` "Denormal / underflow soundness".

**ODE performance baseline**: CAPD order-20 was at or below Codac CtcLohner runtime on all tested ODE benchmarks (cardiac, prostate, bouncing-ball families) — see `docs/decisions.md` "ODE backend". The `--capd-t-gate` / `--capd-ndim-gate` CLI flags have been removed — they were only meaningful under the old Codac/CAPD gated hybrid.
