...
**Rounding-mode gate (`./rounding_debug_gate.sh`).** The FPU-rounding invariants are only checked in a Debug build (`DREAL_ASSERT_ROUNDING` compiles out under `NDEBUG`). This script builds the Debug test target (`cmake-build-debug`) and runs the suite, failing on any rounding-assertion abort; it runs `lint.py` (the static source-hygiene lint) first. Run it before merges, alongside `./copy_lint.sh` (the incremental clang-tidy copy + UB/perf gate — see the source-hygiene note under Key Design Notes). See the "FPU rounding mode" section under Key Design Notes.
...
- **IBEX** (`ncsys-lab/ibex-lib@dreal-perf-patches`, source-built via ExternalProject from `${IBEX_GIT_REPOSITORY}` defaulting to `https://github.com/ncsys-lab/ibex-lib.git`; override to `file:///path/to/ibex-fork` for local-dev iteration). 12 surgical patches on top of mainline `ibex-team/ibex-lib` (lazy-grad, backward callback, parser.yc ADL fix, mathlib arm64-Linux, 3 callback audit fixes for vector/matrix args, reference aliasing, and `EmptyBoxException` precision, gaol `log`/`pow` soundness, the two gaol ARM64 rounding-cost levers — inline FPCR write #9 + batched nearest-region #10, #11 replacing `HC4Revise`'s `EmptyBoxException` control flow with a bool return-status so `__cxa_throw` unwinding leaves the backward hot path entirely, and #12 `underflow_saturate` so the tight-inverting HC4 backward ops — `bwd_pow`/`exp`/`sqr`/`mul`/`div` — stop false-`unsat`'ing on subnormal-band targets, dreal/dreal4#321); see `../ibex-fork/MIGRATION.md`. Installed into `gcc_build/ibex-install/`. `CMakeLists.txt` pins the fork sha (`d9930909`).
...
**`upgrade-ibex`** (current): The post-Codac-elimination architecture, on top of `tacas26-odes`. IBEX is source-built from `ncsys-lab/ibex-lib@dreal-perf-patches` (12 surgical patches catalogued in `../ibex-fork/MIGRATION.md`; `CMakeLists.txt` pins sha `d9930909`). CAPD master is the sole ODE backend.
- ODE contractor in `contractor_odes.cc`: `run_capd_fwd`/`run_capd_bwd` (CAPD `IOdeSolver` order-20, tunable `kCapdTaylorOrder`) integrate `f(x)` / `-f(x)` over `[0, t_ub]` and return the **time-ordered trajectory tube as per-slice enclosures** (cav26's `compute_enclosures`, sub-gridded `kHullGrid=16` per adaptive step) — NOT a coarse component-wise hull. `contractor_ode_lohner::Prune` then runs cav26's per-slice filter where the box/invariant logic lives: walk slices in time order, (1) check the ForallT invariant on each slice (write its state into `m_vars_t`, run the HC4 invariant contractors; the first slice they empty = a trajectory-interior violation → stop), (2) intersect each terminal-eligible slice (time ∈ `[t_lb, t_ub]`) with the X_t gate, keep non-empty, (3) hull survivors → narrowed X_t, hull their times → narrowed time var; **no survivor → `set_empty()`** (sound refutation). The per-slice form (not a hull) is what makes interior-only invariant violations, anti-correlated tubes (no single time hits the gate), and time narrowing work — a hull collapses the per-time/per-component correlation and under-refutes (false delta-sat). Integration time may be a variable, a `RealConstant` interval, or an exact constant. Any integrator exception (step-control divergence, or a mid-enclosure singularity like CAPD `IntervalError` "possible division by zero" on the sigmoid-inverter flows) → **skip** for that call: all caught, never rethrown — there is no ICP-level contractor catch, so an escaping exception would `terminate()` the whole solve (cav26 nominally rethrew, relying on a catch this architecture lacks). `run_capd_trace` for `--visualize`. This whole per-slice filter was lost in the Codac→CAPD rewrite, whose `run_capd_fwd` intersected the terminal box with only the enclosure at the **single endpoint** `t_ub` — an unsound over-narrowing that **false-`unsat`s free-time integrals** whose solution is reached at an interior time `< t_ub` (proven: `github_oct5_0hz_k2_prostate_cancer_*` — committed HEAD → `unsat` 0.03 s, while cav26 and the restored per-slice form → `delta-sat` with a witness at interior times ≈1.7–4.4 « horizon 20; dReal3 segfaults on these inputs, so cav26 + witness are the oracle). Restored to cav26 fidelity; see `contractor_odes_semantic_test.cc` (`GravityInvariantTest`=interior invariant, `AntiCorrelatedTest`=correlation refutation, `DecayFlowTest.FwdInfeasible_BoxEmpties`/`FwdFeasible_TimeNarrows`/`FwdConstantTime_*`).
...
- `select.py` — picks 8 **family-weighted** random benchmarks + all current anomalies; outputs TSV (csv_name TAB filepath). `--family a,b,c` restricts the corpus to those families (`odeexpr,saradc,github,tacas`) before selection; `--all` emits **every** benchmark of the filtered corpus deterministically (no random, no anomalies) — for an A/B over a fixed set, e.g. `select.py --family github,tacas,saradc --all` (the 123 ODE-family jobs; odeexpr is ODE-free so it's excluded when A/B-ing an ODE-contractor change)
- `do_ab.sh BIN_A BIN_B [jobs_file]` — A/B two solver builds over the **same** jobs (default = the full ODE family). Runs them **sequentially** (never concurrently — overlapping batches starve jobs and turn real solves into false wall-clock TIMs) via `run_batch.sh` (`DREAL_BINARY`), parses each, and emits a `compare_solvers.py` table. The sanctioned way to compare e.g. committed-HEAD vs a working-tree build (don't hand-roll an A/B harness)
...
**Source-hygiene lint (`lint.py`) — regex routing/shape rules; copies + UB/perf are a separate clang-tidy gate.** `lint.py` is a dependency-free regex check (the syntactic intent of a clang-tidy *custom* check, realized without the LLVM/libTooling build — `CMAKE_CXX_CLANG_TIDY` stays unset, nothing runs in CI). Run it with `python3 lint.py` (static, no build) or `./rounding_debug_gate.sh` (lint + Debug ctest gate). It cannot see C++ **copies** (a copy is semantic, not syntactic — `auto x = f()` copies or not by `f`'s return type, regex-invisible); those are caught by **`./copy_lint.sh`**, a scoped gate over clang-tidy's *built-in* checks (check set + `src/dreal/` header filter in `.clang-tidy`). The set is all `performance-*` (copy-detection + other perf pitfalls) **minus `enum-size`**, plus an explicit allow-list of the **UB / memory-safety / segfault-class** `bugprone-*` checks (`use-after-move`, `dangling-handle`, `undefined-memory-manipulation`, ptr/array mismatches, `sizeof` misuse, …) — *not* the style/intent bugprone checks (`easily-swappable-parameters`, `narrowing-conversions`, …) or the FP-prone `unchecked-optional-access`, which are noise here. The whole tree is currently clean on every gated check, so the UB checks cost zero noise and exist purely to catch a future regression; see `.clang-tidy`'s header for the two deliberate exclusions and their reasons. `copy_lint.sh` is **incremental** (only `.cc` changed vs. the `main` merge-base; `--all` for a full sweep), needs brew `llvm`'s clang-tidy plus the macOS `--extra-arg=-isysroot $(xcrun --show-sdk-path)` fixup against the Apple compile DB (`cmake-build-debug/compile_commands.json`); a flagged finding is fixed (const&/move / the real bug) or justified inline with `// NOLINT(<check>) <reason>` (the type-aware twin of `// lint: allow`). `lint.py` itself forbids, in `src/dreal/`: **(routing)** raw `ibex::Function::backward` (route through `ibex_hc4_backward`), raw `.mid()`/`.diam()` (route through `safe_mid`/`safe_diam`) — all in `util/rounded_interval.h` — intervals built from hand-written scalar `+`/`-` arithmetic (use `make_sound_interval`), raw json `.dump(` (route through `dump_json`, `util/json_guarded.h`), and raw `std::to_string` on a value feeding a parser — it truncates a double to 6 fractional digits (`%f`), the `to_capd_string` false-`unsat` (use `to_capd_string` for the CAPD feed or `format_double` for display; integer counters/IDs carry `// lint: allow int`); **(iteration shape)** a subscript by a side-effecting counter (`arr[i++]`/`arr[++i]`) — a parallel index that drifts out of sync with a range-`for` loop variable, the BUG-005 scrambled-model class (commit `5774191f2`, fixed `0296a8e19`); use one shared index. Scalar `double`→decimal *display* routing (`os << v`) is **not** regex-detectable, so it is enforced at *compile time* by `format_double`/`to_capd_string` requiring the `NearestRounding` token, not by the lint (the lint catches only the regex-detectable `std::to_string` solver-feed trap). Legitimate exceptions carry an inline `// lint: allow <reason>` marker; the parser/scanner sources and the wrapper-definition files (`rounded_interval.h`, `rounded_format.h`, `json_guarded.h`) are skipped.
...
**ODE feed faithfulness (`to_capd_string` precision)** — the vector field CAPD integrates must be a *faithful* image of the symbolic RHS, or the per-slice filter refutes against the wrong dynamics. `to_capd_string(double)` (`src/dreal/contractor/odes/to_capd_string.h`) renders every constant at `std::numeric_limits<double>::max_digits10` (**17**) significant digits, so CAPD's interval-parse of the decimal literal brackets the exact double. It previously used `std::to_string`, which emits only **6** fractional digits (`sprintf %f`): a coefficient like `1/3 → "0.333333"` made CAPD integrate `3·(1/3)` as `0.999999` — a vector field unfaithful by `1e-6`. A clock whose terminal gate sits at the integration-window end (`tau=1` at `t=t_ub`) then has no surviving terminal slice → **false-`unsat`** (the `k32/k64_water` + `k64_thermostat` triple-network automata; `ode_soundness_repros/ws_taupin.smt2`). The bug was longstanding and **shared** by `main`/cav26's source — their looser filters *masked* the 1e-6 deficit; the tighter per-slice tube *exposed* it (so the per-slice filter is sound; the feed lied). Decimal formatting is correctly rounded only in `FE_TONEAREST`, so `to_capd_string` carries a `DREAL_ASSERT_ROUNDING(FE_TONEAREST)` (it is always reached under `make_capd_ode_cache`'s `NearestRoundingScope`; direct-call unit tests must establish nearest themselves — `ToCapdStringTest`'s fixture does). Scientific notation (|v|<1e-4 or huge) is re-rendered as `fixed`-`setprecision(40)` because CAPD's parser is unreliable on `1e-3` forms. Tests: `to_capd_string_test.cc::ConstantRoundTripsExactly`; the end-to-end refutation flip is `contractor_capd_test.cc::{CapdFwd,CapdBwd}` (cumulative-gaussian witness `p=Φ(10)−Φ(-10)<1`, which the truncated field wrongly refuted).

**Denormal/underflow soundness (dreal/dreal4#321)** — a value like `2^-1075` (`pow(0.5,1075)`) underflows, surfacing in **two layers** (a genuine false-`unsat` in Drake folding, and a subnormal-scale delta-completeness imprecision in the ibex backward):
- **ibex HC4 backward** (fork patch #12, `underflow_saturate` in `../ibex-fork`): a forward op soundly over-approximates an underflowed result up to the subnormal ceiling (`pow(0.5,1075) → [0, DBL_TRUE_MIN]`), but the backward ops that invert via a *tight* gaol primitive (`bwd_pow`/`bwd_exp`/`bwd_sqr`/`bwd_mul`/`bwd_div`) were tighter than the forward and emptied a feasible operand on a subnormal-band target. `underflow_saturate(y)` widens a target lying *entirely* in the subnormal band to include 0 before the tight inverse (keying on the endpoint farthest from 0, so `[DBL_TRUE_MIN,+inf]` is left alone — that is what keeps ibex's `bwd_div08` etc. unchanged). Audited siblings `bwd_sqrt`/`bwd_log`/`bwd_root` invert via a loose forward op and were already sound. **Accepted tradeoff:** the box being emptied is crisp-unsat but *delta*-sat (the true solution is within δ of the box), so this is a delta-completeness imprecision, not a crisp soundness bug. `underflow_saturate` *weakens* the backward contractor at the subnormal scale, which can also stop *legitimate* pruning of robustly-unsat subnormal infeasibility — observed once as `tacas_c2e2_0hz_k7_..._inverter_sigmoid_UNS` flipping UNSAT→delta-sat. That is acceptable by design: dReal is sound but only delta-complete, so a false `unsat` is catastrophic while an over-permissive delta-sat is allowed. (Do not re-investigate that benchmark flip.)
- **Drake constant fold** (`sound_constant_fold` in `symbolic_expression.cc`): the SMT2 parser builds an *exactly-representable* literal (`0.5`, integers) as a `Constant` (double) and an *inexact* decimal (`0.1`) as a `RealConstant` (interval) — `parser.yy`'s `DOUBLE`/`INT` rules. So `pow(0.5,1075)` is `pow(Constant,Constant)` and eagerly folds via `std::pow`, where `2^-1075` rounds to `0.0` — a *lying* literal that makes `pow(0.5,1075) > 0` a false `unsat`. `sound_constant_fold` (applied at the `pow`/`mul`/`div` fold sites) detects an *unfaithful* fold (nonzero true value underflowed to ±0, or finite overflowed to ±inf — sign read from `std::signbit`) and folds to a sound **`RealConstant` interval** bracketing the true value (`[0, DBL_TRUE_MIN]`, `[-DBL_TRUE_MIN, 0]`, `[DBL_MAX, +inf]`, `[-inf, -DBL_MAX]`) instead of the scalar. This stays a *constant* — crucial because a `Pow`/`Mul` with all-`Constant` operands violates a Drake AST invariant (`ExpressionMulFactory::AddTerm` asserts it; an earlier "keep it symbolic" attempt aborted the Debug build) — yet is sound. (`RealConstant` requires a non-degenerate one-ULP bracket: `lb < ub` and `ub == nextafter(lb)`; the four underflow/overflow brackets all satisfy this.)
- Tests: `test/dreal/contractor/test/ibex_log_pow_edge_cases_test.cc` (`SubnormalBackwardStaysConsistent`, `PowSubnormalUnderflowEndToEnd`); `test/dreal/symbolic/test/denorm_constant_fold_test.cc` (the underflow/overflow folds produce sound RealConstant intervals, faithful folds still scalar); `test/dreal/api/test/gaol_directed_rounding_false_unsat_test.cc` (`DenormUnderflowEndToEnd`, end-to-end SAT); `test/dreal/smt2/test/denorm_underflow_smt2_test.cc` (parse-and-check of real SMT2 text).
