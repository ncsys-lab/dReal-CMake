...
**Routing lint (`rounding_lint.py`).** A dependency-free static check (the syntactic intent of a clang-tidy custom check, without the LLVM/libTooling build) that forbids, in `src/dreal/`: raw `ibex::Function::backward` (route through `ibex_hc4_backward`, `util/ibex_guarded.h` — a token-gated wrapper so the gaol arithmetic itself requires `UpwardRounding`), raw `.mid()`/`.diam()` (route through `safe_mid`/`safe_diam`), and intervals built from hand-written scalar `+`/`-` arithmetic (use `make_sound_interval`). Legitimate exceptions carry an inline `// rounding-lint: allow <reason>` marker (model printing, integer bisection); the parser and the wrapper-definition files are skipped. `./rounding_debug_gate.sh` runs the lint first, then the Debug-build assertion gate.
